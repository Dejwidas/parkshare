...
var f = {
  ...
  timeToMin: function(t) { var p=t.split(":").map(Number); return p[0]*60+p[1]; },
};
      ...
      on: function(event, filter, cb) {
        ...
        return {
          subscribe: function() {
            ...
            return {
              unsubscribe: function() { try { ws.close(); } catch(e) {} }
            };
          }
        };
      }
...
var c = {
  ...
  calCell:function(has,past,sel,conflict){return{borderRadius:6,aspectRatio:"1",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:500,cursor:past||conflict?"default":"pointer",opacity:past?0.3:1,background:sel?"#7c3aed":conflict?"#2d1515":has?"#065f46":"transparent",color:sel?"#fff":conflict?"#f87171":has?"#6ee7b7":"#4b5563",border:"1px solid "+(sel?"#7c3aed":conflict?"#7f1d1d":has?"#065f46":"#1f2230")};},
...
function Footer() {
  return React.createElement("div", {style:{textAlign:"center",padding:"20px 0 8px"}},
    React.createElement("div", {style:{fontSize:11,color:"#374151"}},
      "v0.26 · ",
      React.createElement("a", {href:"mailto:kontakt@parkshare.pl",style:{color:"#4b5563",textDecoration:"none"}}, "kontakt@parkshare.pl")
    )
  );
}
...
function ParkLogo(props) {
  ...
  return React.createElement("div", {
    style:{width:size,height:size,borderRadius:Math.round(size*0.22),background:"#7c3aed",display:"inline-flex",alignItems:"center",justifyContent:"center",flexShrink:0}
  },
    React.createElement("span", {
      style:{fontSize:Math.round(size*0.58),fontWeight:800,color:"#fff",fontFamily:"system-ui,sans-serif",lineHeight:1}
    }, "P")
  );
}

function ConfirmDialog(props) {
  var msg=props.msg; var onConfirm=props.onConfirm; var onCancel=props.onCancel;
  return React.createElement("div", {style:{...c.overlay,zIndex:300}, onClick:onCancel},
    React.createElement("div", {style:{...c.modal,maxWidth:320}, onClick:function(e){e.stopPropagation();}},
      React.createElement("div", {style:{fontSize:14,color:"#e8eaf0",marginBottom:20,lineHeight:1.5}}, msg),
      React.createElement("div", {style:{display:"flex",gap:8}},
        React.createElement("button", {style:{...c.btn("primary"),flex:1}, onClick:onConfirm}, "Tak"),
        React.createElement("button", {style:{...c.btn("default"),flex:1}, onClick:onCancel}, "Anuluj")
      )
    )
  );
}
...
function GroupSwitcher(props) {
  var groupId=props.groupId; var groupName=props.groupName; var user=props.user; var onJoin=props.onJoin; var onNew=props.onNew;
  ...
  return React.createElement("div", {style:{position:"relative"}},
    React.createElement("div", {style:{cursor:"pointer",display:"flex",alignItems:"center",gap:6}, onClick:function(){setOpen(function(o){return !o;});setShowJoinForm(false);setShowNewForm(false);}},
      React.createElement("div", null,
        React.createElement("div", {style:{fontSize:16,fontWeight:600,color:"#a78bfa",display:"flex",alignItems:"center",gap:8}},
          React.createElement(ParkLogo, {size:22}),
          React.createElement("span", null, "ParkShare")
        ),
        React.createElement("div", {style:{display:"flex",alignItems:"center",gap:4}},
          React.createElement("span", {style:{fontSize:11,color:"#6b7280"}}, groupName),
          React.createElement("span", {style:{fontSize:9,color:"#4b5563"}}, "v")
        )
      )
    ),
    open && React.createElement("div", {style:dropdownStyle},
      myGroups.length>0 && React.createElement("div", {style:{marginBottom:8}},
        React.createElement("div", {style:{fontSize:11,fontWeight:600,color:"#6b7280",marginBottom:6,textTransform:"uppercase",letterSpacing:"0.5px"}}, "Twoje grupy"),
        myGroups.map(function(g){
          ...
          return React.createElement("div", {
            ...
            React.createElement("div", null,
              React.createElement("div", {style:{fontSize:13,fontWeight:500,color:isActive?"#a78bfa":"#e8eaf0"}}, g.name),
              React.createElement("div", {style:{fontSize:11,color:"#4b5563"}}, g.id)
            ),
            isActive && React.createElement("span", {style:{fontSize:11,color:"#a78bfa"}}, "aktywna")
          );
        }),
        React.createElement("div", {style:{borderTop:"1px solid #2a2d3e",margin:"8px 0"}})
      ),
      !showJoinForm && !showNewForm && React.createElement("div", {style:{display:"flex",flexDirection:"column",gap:6}},
        React.createElement("button", {style:{...c.btn("ghost"),width:"100%",fontSize:12,padding:"8px"}, onClick:function(){setShowJoinForm(true);}}, "+ Dolacz do grupy (kod)"),
        React.createElement("button", {style:{...c.btn("default"),width:"100%",fontSize:12,padding:"8px"}, onClick:function(){setShowNewForm(true);}}, "+ Utworz nowa grupe")
      ),
      showJoinForm && React.createElement("div", null,
        React.createElement("input", {style:{...c.input,marginBottom:6,fontSize:13}, placeholder:"Wpisz kod grupy", value:code, onChange:function(e){setCode(e.target.value.trim());}}),
        err && React.createElement("div", {style:{fontSize:11,color:"#f87171",marginBottom:6}}, err),
        React.createElement("div", {style:{display:"flex",gap:6}},
          React.createElement("button", {style:{...c.btn("primary"),flex:1,fontSize:12,padding:"7px"}, onClick:join}, "Dolacz"),
          React.createElement("button", {style:{...c.btn("default"),fontSize:12,padding:"7px"}, onClick:function(){setShowJoinForm(false);setErr("");setCode("");}}, "Anuluj")
        )
      ),
      showNewForm && React.createElement("div", null,
        React.createElement("input", {style:{...c.input,marginBottom:6,fontSize:13}, placeholder:"Nazwa osiedla", value:newName, onChange:function(e){setNewName(e.target.value);}}),
        React.createElement("div", {style:{display:"flex",gap:6}},
          React.createElement("button", {style:{...c.btn("primary"),flex:1,fontSize:12,padding:"7px"}, onClick:createNew}, "Utworz"),
          React.createElement("button", {style:{...c.btn("default"),fontSize:12,padding:"7px"}, onClick:function(){setShowNewForm(false);setNewName("");}}, "Anuluj")
        )
      )
    ),
    open && React.createElement("div", {style:{position:"fixed",top:0,left:0,right:0,bottom:0,zIndex:199}, onClick:function(){setOpen(false);setShowJoinForm(false);setShowNewForm(false);}})
  );
}

function ShareModal(props) {
  var group=props.group; var onClose=props.onClose;
  ...
  if(verifyMsg) return (
    <div style={{minHeight:"100vh",background:"#0f1117",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24}}>
      ...
      <div style={{...c.card(true),maxWidth:360,width:"100%",textAlign:"center"}}>
        ...
        <div style={{fontSize:13,color:"#6b7280",marginBottom:16,lineHeight:1.6}}>Wyslalismy link weryfikacyjny na adres<br/><span style={{color:"#a78bfa"}}>{email}</span><br/><br/>Kliknij w link w e-mailu a nastepnie zaloguj sie.</div>
...
function AdminPanel(props) {
  var groupId=props.groupId; var user=props.user; var spots=props.spots; var slots=props.slots; var onClose=props.onClose; var onDataChange=props.onDataChange;
  ...
  async function loadMembers(){try{var rows=await sb.from("user_groups").select("*","&group_id=eq."+groupId+"&order=role.desc");setMembers(rows);}catch(e){}finally{setLoading(false);}}
  async function loadGroup(){var g=await sb.from("groups").select("*","&id=eq."+groupId);if(g.length){setGroupName(g[0].name);setNewName(g[0].name);}}
  useEffect(function(){loadMembers();loadGroup();},[]);
  async function setRole(userId,role){try{await sb.from("user_groups").update({role:role},"?user_id=eq."+userId+"&group_id=eq."+groupId);showToast(role==="admin"?"Nadano admina":"Odebrano admina");loadMembers();}catch(e){showToast("Blad","error");}}
  async function removeMember(userId,userName){try{await sb.from("user_groups").delete("?user_id=eq."+userId+"&group_id=eq."+groupId);showToast(userName+" usuniety","warn");loadMembers();onDataChange();}catch(e){showToast("Blad","error");}}
  async function removeSpotAdmin(spotId){try{var ss=slots.filter(function(sl){return sl.spot_id===spotId;});for(var i=0;i<ss.length;i++)await sb.from("slots").delete("?id=eq."+ss[i].id);await sb.from("spots").delete("?id=eq."+spotId);showToast("Miejsce usuniete","warn");onDataChange();}catch(e){showToast("Blad","error");}}
  async function cancelBookingAdmin(slotId){try{await sb.from("slots").update({booked:false,booked_by:null,booker_phone:null,booked_at:null,booked_by_uid:null},"?id=eq."+slotId);showToast("Rezerwacja anulowana","warn");onDataChange();}catch(e){showToast("Blad","error");}}
  async function saveGroupName(){if(!newName.trim())return;try{await sb.from("groups").update({name:newName.trim()},"?id=eq."+groupId);setGroupName(newName.trim());setEditingName(false);showToast("Nazwa zaktualizowana");onDataChange();}catch(e){showToast("Blad","error");}}
  return (
    <div style={c.overlay} onClick={onClose}>
...
function ParkApp(props) {
  var groupId=props.groupId; var user=props.user; var onLeave=props.onLeave; var onSwitchGroup=props.onSwitchGroup; var onNew=props.onNew;
  ...
  async function loadAll(){
    try{
      ...
      if(!user.guest){var ug=await sb.from("user_groups").select("role","&user_id=eq."+user.uid+"&group_id=eq."+groupId);if(ug.length)setMyRole(ug[0].role);}
  ...
  function confirm(msg,onConfirm){setConfirmDialog({msg:msg,onConfirm:onConfirm});}
  ...
  // Check if a date has conflicting slots for a spot
  function hasConflict(spotId, dateKey) {
    var existing = slotsForSpot(spotId).filter(function(sl){return sl.date===dateKey;});
    return existing.length > 0;
  }

  // Pending bookings for my spots (booked but still in cancel window = pending)
  ...
  async function updateField(spotId,field,val){try{await sb.from("spots").update({[field]:val},"?id=eq."+spotId);loadAll();}catch(e){}}
  ...
  async function addSlots(spotId){
    ...
    var conflicts=multiDates.filter(function(d){return hasConflict(spotId,d);});
    if(conflicts.length>0){showToast("Wybrany termin juz istnieje dla tego miejsca. Usuń konflikty i spróbuj ponownie.","error");return;}
  ...
  async function saveEditedSpot(spotId,data){
    try{
      await sb.from("spots").update({name:data.name,desc:data.desc,owner:data.owner,phone:data.phone,email:data.email,note:data.note,type:data.type,spot_visibility:data.spotVisibility},"?id=eq."+spotId);
      setEditSpotModal(null);showToast("Miejsce zaktualizowane!");loadAll();
    }catch(e){showToast("Blad: "+e.message,"error");}
  }

  function SlotCal(props2) {
    var spotId=props2.spotId;
      ...
      <div>
        ...
        <div style={c.calGrid}>
          ...
          {cells.map(function(day,i){
            ...
            var conflict=spotId&&!sel&&hasConflict(spotId,k);
            return <div key={k} style={c.calCell(has,past,sel,conflict&&!has)} title={conflict&&!has?"Ten dzien juz ma termin":""} onClick={function(){if(!past&&!conflict) toggleMulti(k);}}>{day}</div>;
          })}
        </div>
          ...
          <div>
            ...
            {pendingBookings.length>0&&(
              <div style={{background:"#1c1200",border:"1px solid #fbbf24",borderRadius:10,padding:"12px 14px",marginBottom:16}}>
                <div style={{fontSize:12,fontWeight:600,color:"#fbbf24",marginBottom:8}}>Nowe rezerwacje oczekujace na potwierdzenie ({pendingBookings.length})</div>
              ...
              <div style={{display:"flex",alignItems:"center",gap:10,padding:"7px 0"}}>
                ...
                <div>
                  ...
                  {(function(){
                    ...
                    return <div><div style={{fontSize:12,color:"#374151",marginBottom:4}}>Numer jest ukryty — pojawi sie automatycznie po zatwierdzeniu rezerwacji.</div>{contactModal.phone&&<div style={{fontSize:12,color:"#6b7280",marginTop:4}}>Mozesz zapytac wlasciciela: <span style={{color:"#a78bfa"}}>{contactModal.phone}</span></div>}</div>;
                  })()}
                </div>
              </div>
            </div>
      ...
      {bookModal&&(
        <div style={c.overlay} onClick={function(){setBookModal(null);}}>
          <div style={c.modal} onClick={function(e){e.stopPropagation();}}>
            ...
            <div style={{background:"#0d2a1e",border:"1px solid #065f46",borderRadius:8,padding:"8px 12px",fontSize:12,color:"#6ee7b7",marginBottom:14}}>Wlasciciel ma 1 godzine na anulowanie. Po tym czasie rezerwacja jest automatycznie zatwierdzona.</div>
...
export default function Root() {
  ...
  if(saved){_authToken=saved.token;}

  function handleAuth(u){_authToken=u.token||null;saveSession(u,u.token||"");setUser(u);setScreen("landing");}

  async function handleJoin(id){
    if(user&&!user.guest){try{await sb.from("user_groups").upsert({user_id:user.uid,group_id:id,role:"member",user_name:user.name,user_email:user.email},"user_id,group_id");}catch(e){}}
  ...
  async function handleLogout(){try{await sb.auth.signOut();}catch(e){}clearSession();setUser(null);setScreen("auth");}
