...
function sbHeaders(token) {
  var h = {
    ...
    "apikey": SUPABASE_KEY,
    "Authorization": "Bearer " + (token || _authToken || SUPABASE_KEY),
    "Prefer": "return=representation"
  };
  return h;
}

async function sbReq(method, table, qs, body, token) {
  var url = SUPABASE_URL + "/rest/v1/" + table + (qs || "");
  var opts = { method: method, headers: sbHeaders(token) };
  ...
  auth: {
    ...
    signOut: async function() {
      await fetch(SUPABASE_URL+"/auth/v1/logout", {
        method: "POST",
        headers: { "Content-Type":"application/json", "apikey": SUPABASE_KEY, "Authorization": "Bearer "+(_authToken||"") }
      });
      _authToken = null;
    }
  },
        ...
        return {
          subscribe: function() {
            ...
            socket.onopen = function() {
              socket.send(JSON.stringify({ topic:"realtime:"+name, event:"phx_join", payload:{ config:{ broadcast:{self:true}, postgres_changes:[{event:"*",schema:"public"}] } }, ref:"1" }));
            };
...
var c = {
  ...
  btn: function(v) { var m={primary:["#7c3aed","#fff"],ghost:["transparent","#7c3aed"],danger:["#3b0d0d","#f87171"],success:["#064e3b","#6ee7b7"],warn:["#451a03","#fbbf24"],default:["#1f2230","#9ca3af"]}[v||"default"]; return {padding:"8px 14px",borderRadius:8,border:"none",cursor:"pointer",fontSize:13,fontWeight:500,background:m[0],color:m[1]}; },
...
function AuthScreen({ onAuth }) {
  ...
  async function register() {
    ...
    try {
      await sb.auth.signUp(email.trim(), pass, name.trim());
      setVerifyMsg(true);
    } catch(e) { setErr("Błąd rejestracji: "+e.message); }
  ...
  async function login() {
    ...
    try {
      ...
      var userName = (data.user && data.user.user_metadata && data.user.user_metadata.name) || email;
      onAuth({ uid: data.user.id, name: userName, email: data.user.email, guest: false, token: data.access_token });
    } catch(e) {
      var msg = e.message;
  ...
  var inp = function(val, set, ph, type) {
    return <input style={{...c.input, marginBottom:10}} type={type||"text"} placeholder={ph} value={val} onChange={function(e){set(e.target.value);}}/>;
  };
    ...
    <div style={{minHeight:"100vh",background:"#0f1117",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24}}>
      ...
      <div style={{...c.card(true),maxWidth:360,width:"100%",textAlign:"center"}}>
        ...
        <div style={{fontSize:13,color:"#6b7280",marginBottom:16,lineHeight:1.6}}>
          ...
          Kliknij w link w e-mailu, a następnie wróć tutaj i się zaloguj.
        </div>
    ...
    <div style={{minHeight:"100vh",background:"#0f1117",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24}}>
      ...
      <div style={{width:"100%",maxWidth:340}}>
        ...
        <div style={c.card(true)}>
          ...
          {inp(pass,setPass,"Hasło (min. 6 znaków)","password")}
...
function Landing({ user, onJoin, onNew, onLogout }) {
  ...
  async function join() {
    ...
    try {
      ...
      if (!user.guest) {
        await sb.from("user_groups").upsert({user_id:user.uid,group_id:t}, "user_id,group_id");
      }
      setErr(""); onJoin(t);
...
function ShareModal({ group, onClose }) {
  ...
  var link = "parkshare.app/g/"+group.id;
...
function ParkApp({ groupId, user, onLeave }) {
  ...
  function slotsForSpot(spotId){ return slots.filter(function(sl){return sl.spot_id===spotId;}); }
  function slotsOn(spotId,d){ return slotsForSpot(spotId).filter(function(sl){return sl.date===d;}); }
  var dk = f.dateKey(selDate);
  var weekDays = Array.from({length:7},function(_,i){return f.addDays(today,i);});
  ...
  async function removeSlot(slotId){
    try{ await sb.from("slots").delete("?id=eq."+slotId); loadAll(); }catch(e){}
  }

  async function cancelBooking(slotId){
    try{ await sb.from("slots").update({booked:false,booked_by:null,booker_phone:null,booked_at:null,booked_by_uid:null},"?id=eq."+slotId); showToast("Rezerwacja anulowana.","warn"); loadAll(); }catch(e){}
  }
                ...
                <div style={{marginBottom:10}}>
                  ...
                  <div style={{display:"flex",gap:8}}>
                    {[["underground","🏗 Garaż podziemny"],["outdoor","🌤 Naziemne"]].map(function(row){
                      var val=row[0],lbl=row[1];
                      return <button key={val} style={{...c.btn(newSpot.type===val?"primary":"default"),flex:1,fontSize:12}} onClick={function(){setNewSpot(function(p){return {...p,type:val};});}}>{lbl}</button>;
                    })}
                  </div>
                </div>
                <div style={{marginBottom:14}}>
                  <label style={c.label}>Notatka / płatność</label>
                  <textarea style={c.textarea} placeholder="np. Preferuję BLIK, klucz u portiera..." value={newSpot.note} onChange={function(e){var v=e.target.value; setNewSpot(function(p){return {...p,note:v};});}}/>
                </div>
                <div style={{display:"flex",gap:8}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:10,padding:"7px 0"}}>
                ...
                <div>
                  ...
                  {(function(){
                    ...
                    var show=!user.guest&&(isOwner(contactModal)||mySlot);
                    return show
      ...
      {bookModal&&(
        <div style={c.overlay} onClick={function(){setBookModal(null);}}>
          <div style={c.modal} onClick={function(e){e.stopPropagation();}}>
            ...
            <div style={{display:"flex",gap:8}}>
              <button style={c.btn("primary")} onClick={confirmBook}>Potwierdź</button>
...
export default function Root() {
  ...
  async function handleJoin(id) {
    if (user && !user.guest) {
      try { await sb.from("user_groups").upsert({user_id:user.uid,group_id:id},"user_id,group_id"); } catch(e){}
    }
  ...
  async function handleNew(name) {
    ...
    try {
      ...
      if(user&&!user.guest) await sb.from("user_groups").upsert({user_id:user.uid,group_id:id},"user_id,group_id");
      setActiveGroupId(id); setScreen("app");
